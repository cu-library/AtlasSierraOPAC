...
function M.authorAssertEquals(actual, expected)
    if actual ~= expected then
        actual = actual:gsub("%, %d%d%d%d%-%d?%d?%d?%d?", ".")
        actual = actual:gsub(" $", "")
        actual = actual:gsub("%.%.$", ".")
        if actual ~= expected then
            failure( errorMsgEquality(actual, expected), 2 )
...
function M.editionAssertEquals(actual, expected)
    if actual ~= expected then
        expected = expected:gsub("st%.?$", "st ed.")
        expected = expected:gsub("nd%.?$", "nd ed.")
        expected = expected:gsub("rd%.?$", "rd ed.")        
        expected = expected:gsub("th%.?$", "th ed.")

        if (actual:find("edition") ~= nil and expected:find("ed%.") ~= nil) then
        ...
        elseif (actual == "Fifth edition." and expected:find("^5th edition%.?$") ~= nil) then
        elseif (actual == "Fourth edition." and expected:find("^4th edition%.?$") ~= nil) then
        elseif (actual == "Third edition." and expected:find("^3rd edition%.?$") ~= nil) then
        elseif (actual == "Second edition." and expected:find("^2nd edition%.?$") ~= nil) then
        elseif (actual == "First edition." and expected:find("^1st edition%.?$") ~= nil) then
        elseif (expected:find("1st ?e?d?.?") ~= nil and actual == "") then
        elseif (expected == "First ed." and actual == "") then
        elseif (expected == "First ed." and actual == "1st ed.") then
        elseif actual ~= expected then
            failure( errorMsgEquality(actual, expected), 2 )
        end
    end
...
function M.callnumberAssertEquals(actual, expected)
    if actual ~= expected then
        expected = expected:gsub(" ? ?c%.%d$", "")
        expected = expected:gsub(" book$", "")
        expected = expected:gsub(" v%. %d$", "")
        if actual ~= expected then
            failure( errorMsgEquality(actual, expected), 2 )
        end
    end
